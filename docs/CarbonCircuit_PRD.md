...
Separately, and on its own timeline, that same Facility submits a Sustainability Claim for vintage 2026 stating it used 12,400,000 kWh of verified renewable energy — which is exactly its attested annual renewable supply for that year, so the claim is for its full capacity. The ceiling is computed from that **attested capacity** rather than from the figure the claim requests, as Section 2.1 requires: it looks up the `TW` grid factor (0.494 kgCO2e/kWh), multiplies, divides by 1,000, and applies the 1.00 discount factor, giving a ceiling of 6,125.6 tCO2e. Had the Facility instead requested 14,000,000 kWh against that same attested capacity, the ceiling would still have been 6,125.6 — the requested figure never raises it. Because that exceeds 5,000, the claim enters the queue at `critical` priority and will require two Verifier approvals.
