...
import (
	...
	"github.com/carboncircuit/backend/services/credit-ledger-service/internal/config"
	"github.com/carboncircuit/backend/services/credit-ledger-service/internal/repository"
	"github.com/carboncircuit/backend/services/credit-ledger-service/internal/rpc"
	"github.com/carboncircuit/backend/services/credit-ledger-service/internal/service"
	"github.com/carboncircuit/backend/services/credit-ledger-service/internal/upstream"
)

var revision = "dev"

const pingMethod = "/carboncircuit.creditledger.v1.CreditLedgerService/Ping"

func main() {
	if err := run(); err != nil {
		slog.Error("credit-ledger-service failed to start", slog.Any("error", err))
		os.Exit(1)
	}
}
...
func run() error {
	...
	identity, err := upstream.DialIdentity(
		settings.IdentityAddress, settings.CallTimeout, identityCreds,
	)
	if err != nil {
		return err
	}
	...
	issuer := service.NewIssuer(store, ledger, identity, settings.ConsumerGroup, logger)

	consumer, err := kafka.NewConsumer(kafka.ConsumerOptions{
		Brokers:    settings.KafkaBrokers,
		Group:      settings.ConsumerGroup,
		Topics:     []string{events.TopicClaimDecisionRecorded},
		Logger:     logger,
		Handle:     issuer.Apply,
		DeadLetter: producer,
	})
	...
	ledgerServer := rpc.NewCreditLedgerServer(
		store, service.NewReader(store, ledger), logger, revision,
	)

	verifier := servicetoken.NewVerifier(publicKey)
	exempt := map[string]bool{pingMethod: true}

	logger.Info("credit-ledger-service ready",
	...
	return grpcx.Serve(ctx, grpcx.ServerOptions{
		...
		Register: func(server *grpc.Server) {
			creditledgerv1.RegisterCreditLedgerServiceServer(server, ledgerServer)
		},
	})
}
